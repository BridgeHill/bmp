...
/////////////////////////////////////////////////////////////////////////////
// Galois Field 加罗华域 GF(2^8) 
// 域中每个元素形如多项式 aX^7+bX^6+cX^5+dX^4+eX^3+fX^2+gX^1+h
// GF(2^8)名称中2限定系数范围(a~h取值限定为2以下整数0或1)，8限定指数范围.
// (a~h共８位，限取０和１，这个系数组合就和自然二进制对应起来了，可用2表示x+1，4来表示X^3)

// 素多项式 m(X) = X^8+X^4+X^3+X+1 ,域中元素A与B进行加减乘除后的值，mod m(X)后，也会在此域中。
// 域中加法和减法一样,对应项系数进行异或,易知结果也在此域中
// 域中mod,和多项式除法取余类似，过程中的加法采用异或加。
// 域中乘法，过程中的加采用异或加，最后 mod m(X)得到一个值(由上面可知，此值也在域中)

// 乘法操作复杂，不适合程序操作，用查表法代替
// 生成元ｇ(X)=X+1,域中任何元素都可表示成　ｇ(X)^(k%255) mod m(X) //元素0例外
// 所以　域中元素Ａ对应k1,元素Ｂ对应k2,Ａ＊Ｂ对应(k1+k2)
// 建立两张表，正表：元素-〉指数，负表：指数到元素，二表先后使用，可得到二数的乘积。

// 指数到元素,指数由0->255 ,算值 (ｇ(X)^(k%255) mod m(X)) 得到多项式，多项式的系数组合写成一个数
...
static int nIndicatorLenHanzi[]	   = { 8, 10, 12};
...
//按照给定的纠错等级、版本、是否允许自动扩展(提升版本到能容纳下输入数据的最小版本)、掩模图形号(-1为自动选择最佳),对输入字符流的前若干字符(0代表所有)进行编码 
...
//	int ncTerminater = min(4, (ncDataCodeWord * 8) - m_ncDataCodeWordBit);
	int ncTerminater = (4<((ncDataCodeWord * 8) - m_ncDataCodeWordBit))?(4):((ncDataCodeWord * 8) - m_ncDataCodeWordBit);

	if (ncTerminater > 0)
		m_ncDataCodeWordBit = SetBitStream(m_ncDataCodeWordBit, 0, ncTerminater);

    // 最后一个编码后数据码字不足8位，用0填充至8位
    // 使用"11101100, 00010001"交替填充 数据码空间存储完编码后数据后的剩余空间
...
{   
	//根据版本分情况处理:40~27；26~10;9~1;
    //QR_VRESION_S	0 // 1~9
    //QR_VRESION_M	1 // 10~26
    //QR_VRESION_L	2 // 27~40
...
//	ZeroMemory(m_nBlockLength, sizeof(m_nBlockLength));
	...
	//填充两个数组：m_byBlockMode[]  和 m_byBlockLength[]
	//m_byBlockMode[] : 输入流中每个符号代表的类型，索引为符号个数，日文汉字看为一个符号
	//m_byBlockLength[]: 输入流中每个符号所占字节
	for (m_ncDataBlock = i = 0; i < ncLength; ++i)
	{
		...
		if (i < ncLength - 1 && IsHanziData(lpsSource[i], lpsSource[i + 1]))
			byMode = QR_MODE_HANZI;
			...
			(m_byBlockMode[nBlock] == QR_MODE_ALPHABET && m_byBlockMode[nBlock + 1] == QR_MODE_NUMERAL))
		{
			...
			if (ncSrcBits > ncDstBits)
			{
				...
				if (nBlock < m_ncDataBlock - 2 && m_byBlockMode[nBlock + 2] == QR_MODE_8BIT)
				{
					...
					ncJoinBehind = 0;
                //联合前位置或再后位置，三个符号一起用字节模式转化会更短，进行如下处理
				...
				if (nJoinPosition != 0)
				{
					...
					if (nJoinPosition == -1)
					{                     
						 //当前位置符号和前位置符号归类到一起，前位置符号对应的m_byBlockMode不变m_nBlockLength增加
...
//	ZeroMemory(m_byDataCodeWord, MAX_DATACODEWORD);
	...
	for (i = 0; i < m_ncDataBlock && m_ncDataCodeWordBit != -1; ++i)
	{
		...
		else if (m_byBlockMode[i] == QR_MODE_KANJI)// m_byBlockMode[i] == QR_MODE_KANJI
		{
			//按照日文汉字编码
			// 模式指示符(1000b)
            ...
            //每符号化为13bit；ps：日文处理后每个符号对应的 ncData为2
		...
		else
		{
			...
			// 字符计数指示符，位数和放入位置按版本分三种长度处理
			m_ncDataCodeWordBit = SetBitStream(m_ncDataCodeWordBit, (WORD)(m_nBlockLength[i] / 2), nIndicatorLenHanzi[nVerGroup]);

            //每符号化为13bit；ps：中文处理后每个符号对应的 ncData为2
			for (j = 0; j < m_nBlockLength[i] / 2; ++j)
			{
				WORD wBinCode = HanziToBinaly((WORD)(((BYTE)lpsSource[ncComplete + (j * 2)] << 8) + (BYTE)lpsSource[ncComplete + (j * 2) + 1]));
...
int CQR_Encode::GetBitLength(BYTE nMode, int ncData, int nVerGroup)
{
	...
	case QR_MODE_ALPHABET:
		//4位模式指示符+计数指示符+每两个符号化为11bit+余1化6bit
	...
	case QR_MODE_KANJI:
		//4位模式指示符+计数指示符+每符号化为13bit；ps：日文处理后每个符号对应的 ncData为2
		ncBits = 4 + nIndicatorLenKanji[nVerGroup] + (13 * (ncData / 2));
		break;
		
	default: // case QR_MODE_HANZI:
		//4位模式指示符+计数指示符+每符号化为13bit；ps：日文处理后每个符号对应的 ncData为2
		ncBits = 4 + nIndicatorLenHanzi[nVerGroup] + (13 * (ncData / 2));
		break;
	}
...
//将内容wData的0~(ncData-1)位bit 写到m_byDataCodeWord[]的nIndex~(nIndex+ncData-1)位bit空间。。。
...
/////////////////////////////////////////////////////////////////////////////
// CQR_Encode::IsHanziData
// 用于判断源数据中当前位置的后两个字节是否组成一个中文汉字
BOOL CQR_Encode::IsHanziData(unsigned char c1, unsigned char c2)
{
	if( (c1>=0xa1) && (c1<=0xaa) && (c2>=0xa1) && (c2<=0x00fe) )
		return true;
	if( (c1>=0xb0) && (c1<=0xfa) && (c2>=0xa1) && (c2<=0x00fe) )
		return true;
	return FALSE;
}
...
/////////////////////////////////////////////////////////////////////////////
// CQR_Encode::HanziToBinaly
//中文汉字模式下的译码 符号->值

WORD CQR_Encode::HanziToBinaly(WORD wc)
{
	if( ((wc>>8)>=0xa1) && ((wc>>8)<=0xaa) && ((wc&0x00ff)>=0xa1) && ((wc&0x00ff)<=0x00fe) )
	{
		return(WORD)( ((wc>>8)-0xa1) * 0x60 + ((wc&0x00ff) - 0xa1) );
	}
	if( ((wc>>8)>=0xb0) && ((wc>>8)<=0xfa) && ((wc&0x00ff)>=0xa1) && ((wc&0x00ff)<=0x00fe) )
	{
...
// 放置版本信息：共18位，6位数据，12位纠错（数据通过BCH（18，6）编码）
// 相同信息放置在两个区域，冗余 
...
// 放置格式信息
// 15位：5位数据（2位表示纠错等级+3位表示掩模方式）+10位纠错码（BCH（15，5））
// 放置在两个地方，冗余
void CQR_Encode::SetFormatInfoPattern(int nPatternNo)
{
	...
	m_byModuleData[8][m_nSymbleSize - 8] = '\x30'; // 屌掕埫儌僕儏乕儖
